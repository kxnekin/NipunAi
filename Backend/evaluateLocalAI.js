...
app.get("/api/check-ollama", async (req, res) => {
  try {
    ...
    if (response.ok) {
      ...
      res.status(500).json({ 
        status: "ERROR", 
        message: "Ollama responded with error status" 
      });
    }
  } catch (error) {
    res.status(500).json({ 
...
app.post("/api/evaluate", async (req, res) => {
  ...
  try {
    ...
    // More optimized prompt - shorter and faster to process
    const prompt = `
...
Keep responses very concise.`;
    ...
    try {
      ...
      if (!testResponse.ok) {
        console.warn("⚠️ Ollama is not responding, using fallback");
        return res.json(analyzeAnswerFallback(question, answer));
      }
    } catch (testError) {
      console.warn("⚠️ Cannot connect to Ollama, using fallback:", testError.message);
      return res.json(analyzeAnswerFallback(question, answer));
    }

    // Main AI evaluation request with optimized settings
    ...
    const rawText = data.response || data.text || data.output || JSON.stringify(data);
    ...
    console.log("📝 Processing response:", rawText);
    ...
    return res.json(analyzeAnswerFallback(question, answer || "No answer provided"));
  }
});

/* -------------------------------------------------------------------------- */
/* 🔁 Fallback evaluator (local scoring logic if AI fails) */
/* -------------------------------------------------------------------------- */
